...
export class LLMService {
  ...
  async testProviderConnection(): Promise<{ success: boolean; message: string }> {
    try {
      switch (this.config.activeProvider) {
        case 'ollama':
          await axios.get(`${this.config.ollamaUrl}`);
          return { success: true, message: 'Connected to Ollama' };
        
        case 'lmstudio':
          await axios.get(`${this.config.lmStudioUrl}/models`);
        ...
        case 'claude':
          ...
          if (!this.config.claudeKey) throw new Error('Claude API key missing');
  ...
  async generateCompletion(systemPrompt: string, userPrompt: string): Promise<string> {
    try {
      switch (this.config.activeProvider) {
        case 'ollama': {
          const res = await axios.post(`${this.config.ollamaUrl}/api/generate`, {
        ...
        case 'lmstudio': {
          const res = await axios.post(`${this.config.lmStudioUrl}/chat/completions`, {
            messages: [
